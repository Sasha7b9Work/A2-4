...
mabin		DATA	6fh
reacp		DATA	77h	;результат чтени€ асп 4byte int (hex)
...
nuerr		DATA	39h	;номер ошибки (дл€ интерфейса)	
...
bitmas		BIT 	cellbit.1	;20.1 бит заполнени€ массива		
znmat			BIT	cellbit.2	;20.2
bitrs			BIT	cellbit.3	;20.3
bitizm		BIT	cellbit.4	;20.4 вкл реж измерени€
bitavp		BIT	cellbit.5	;20.5  бит режима avp
bitvi11		BIT	cellbit.6	;20.6	бит нажати€ ввод режима вычислить R!!!
...
bitmem		BIT	cellbit1.3	;21.3 нажата кн пам€ть
;bmem11		BIT	cellbit1.4	;21.4  	 бит ѕјћ11
bifl_kt		BIT	cellbit1.5	;21.5  бит дл€ выч  м теста измерени€  
;bmem13		BIT	cellbit1.6	;21.6		 
bitnul		BIT	cellbit1.7	;21.7 бит кнопки Ќ”Ћ№

cellbit2		DATA	22h
bitt2			BIT 	cellbit2.0	;22.0 уст признак Tизм2	нажата кнопка “	
...
bitvich		BIT	cellbit2.4	;22.4  1-е нажатие кн ¬џ„ R (режим ¬џ„ R)
biteizm		BIT	cellbit2.5	;22.5   признак теста измерени€ 
bitpar		BIT	cellbit2.6	;22.6	бит проведенного 1раз парол€ 
bitoll		BIT	cellbit2.7	;22.7 бит перегрузки
;в реж измерени€ прибор измер€ет (кнопка »«ћ не нажата )
;при нажатии кнопок (T,ћ≈Ќё ,ѕјћя“№ ,¬џ„ R)прибор выходит из реж измерени€
;и выдает т.н. буфер индикации	
cellbit3		DATA	23h
knizm			BIT 	cellbit3.0	;22.0 уст признак нажата кнопка »«ћ	
bitmig		BIT 	cellbit3.1	;22.1 бит мигани€ при наборе конст 	
bitakt		BIT	cellbit3.2	;22.2  бит такта
bitznB		BIT	cellbit3.3	;22.3 Ѕ»“ «Ќј ј ѕ–» ¬¬ќƒ≈ константы ¬
...
;hex
rez_A		data 62h	;–≈« сумма 100 (10)отсчетов измерени€	 
rez_A1	data 66h	;–≈«1  среднее по массиву из 2,10 ти элементов int (hex)
rez_A0	data 6ah	;рез изм по нажатию кн Ќ”Ћ№
rez_A2	data 7bh	; A2
...
;hex 
parT	 	data  9ah	;параметр времени измерени€
chbefore	data  9bh	;счетчик предварительного суммировани€
rez_A3  	data  9ch	;A3
;FLOAT
koefKD	data  0a0h	;KD коэфф диапазона
rez_Ak	data  0a4h	;Ak
parol		data  0a8h	;храним в пам€ти пароль
...
parN		data  0afh	;N  пар разр€дности индикатора
...
summa		data  0b3h	;€чейка дл€ суммировани€ 	rez_A
load		data  0b7h	;€ч дл€ загрузки текстов на индикатор
work		data  0b8h	;рабоча€ €чейка
konstA	data  0bch	;введенна€ с экрана константа A
;konstB	data  0c0h	 ;введенна€ с экрана константа B
saven_bl2	data	0c0h	;копи€ адреса последней записанной €чейки блока2
saven_bl3	data  0c2h	;копи€ адреса последней записанной €чейки блока3 
	   ;c4 1bait
Rez_del	data  0c5h	;сумма n-значений /n
rez_R		data	0c9h	;рез вычислени€ R 4baita
interva	data  0cdh	;значение интервала (запись)2baita
chbuf		data  0cfh	;счетчик периода загрузки в буфер и интерфейс
saven_bl	data  0d3h	;номер €чейки в блоке 2 baita
volume	data	0d5h	;объем блока 2 baita
chblok	data	0d6h	;volume;кол измерений блока
copy_hl	data  0d7h	;адрес €чейки блока дл€ просмотра 
saven_bl4	data  0d9h	;копи€ адреса последней записанной €чейки блока4
;data		0dbh e0h-граница стека

;внешнее ќ«” 64K
;в обл внешнего озу 0..ffh читаетс€ страница рпзу и при записи в любую
;€чейку  стираетс€ цела€ страница 0..ffh рпзу
;		XSEG AT 100h
	PUBLIC 	  MASS0
MASS0  xdata 0100h;	скольз€щий массив измерени€
MASS1  xdata 0104h;
MASS2  xdata 0108h;
MASS3  xdata 010ch;
MASS4  xdata 0110h;
...
begin:	mov		sp, #0e0h		;?8
	...
	//P2MDIN=0xff;//11111111;
		...
		mov		TH0,#0f9h;=0xf9;  ”становка таймера
		mov		TL0,#0f9h;=0xf9; 
		mov		EIE1,#80h;=0x80;
...
gener:	;+mov		a, oscxcn
		...
		mov		spi0ckr,#31H;;;;;;;;#04
;установка скорости
		mov		SCON0,#50h;01010000;скорость обмена определ€етс€ таймером1
		mov		PCON,#10000000b;удвоенна€ скорость передачи SMOD0=1
...
;	mov	cnt_in,#0
		...
		mov	IE,#80h ;??80//—н€тие блокировки всех прерываний+RS
;ozu
		clr	a
		mov	r0,#00h	
...
after_2:		
	...
	mov	cnt_out,#0	;??
	mov	cnt_in,#0
	mov	IE,#90h ;—н€тие блокировки всех прерываний+RS
...
goznak:
		mov 		r0,#reacp+3
		mov		a,@r0
		mov		r7,#5		;6 сдвиг на 5 разр€дов вправо
sdvig3:		clr		c
		rrc		a
...
sdvig6:	setb		c
		...
		mov		reacp+1,a
; reacp..reacp+3-результат чтени€ асп(hex) 
...
allsum:	CALL  ldc_ltemp		 ; ltemp <-- 100
		...
		mov		r1,#chbefore
		mov		@r1,#0
		jmp		klmas		;загр в массив скольз€щее ср
nobef99:	inc		@r0		 	;chbefore
		jmp		rebyte;labelA;метка A
;сначала суммируем 100 знач (не выводим на индикатор не расп в скольз€щий массив )
;а потом уже располагаем  в скольз€щий массив  из n
...
noparT4:	mov		@r1,#2		;chmasN n=2
		...
		mov		r0,#rez_A
		call		saver2
;загрузить ј в массив из 2/10 значений
...
endsum:	;23.03	
		...
		inc	@r0
		mov	a,@r0	
		jb	bitmas,masfull		;массив заполнен 2/10 значени€ми
		cjne	a,chmasN,masx5
...
noavp:	;jb	bitvi11,yesvich  ;уст после ввода A B (ввод ¬џ„»—Ћ»“№)
goA2:		;A2=A1-
		mov	r0,#rez_A1
		call	resar2
		mov	r0,#rez_A2
...
gokaA2:	mov	r0,#rez_A1		;(float)
		call	saver2		;это необх дл€ калибровки ,goA2 работает с этим знач
 		jmp	goA2
...
;чтобы уложитьс€ в 5 разр€дов, если отриц располагать
;число в bufind..bufind+5
		mov	r0,#work
		cjne	@r0,#0,outr
		inc	r0			;work+1
		cjne	@r0,#0ffh,bufr3	;6разр€дов нужно вывести 5
		inc	@r0;!!!!!!!!!?
...
bufr2:	mov	a,@r0
		...
		mov	r0,#work+1
		jmp	outr5
;work!=0 значит после дел на 10**9 получили число > с чем 6 разр€дов					
outr:		mov	r0,#adec+9;разместить в буфер индикации R
		...
		subb	a,@r1
		mov	r0,a			;adec+9-n !!!!!!!!!!!!!!!
		mov	r1,#bufind+5;т к число разр€дов > чем знакомест
		mov	r7,#5;7
bufr:		mov	a,@r0
		add	a,#30h
		mov	@r1,a
...
outr5:	mov	a,@r0
		...
		mov	bufind+7,adec+8
		mov	bufind+9,#66h
		orl	bufind+11,#10h;зап€та€ после 1го знакоместа
...
moveA3:	;нажата кн вычR	не выдавать на индикатор не портить bufind..
		...
		;до конца цикла измерени€ результат не размещать в буфере
		;не забывать сбрасывать по выходу из режима обр кнопки 
		;и возврату в режим измерени€
		jb		bitvich,rnorvich
		jb		bitt2,rnorvich
		jb		bitoll,rseeind
		jb		bitmenu,rseeind
...
seegoon:	inc	r0
		mov	@r0,#"A";"E";
		inc	r0
		mov	@r0,#00h ;=feh в нач установе дл€ поджига всех сегм
...
me_5:			;??????????????????
		...
		call	save_hl	;адрес след €чейки внешн озу храним в €чейке saven_bl
		mov			r0,#diap
		mov			a,@r0
		mov			r1,#nblok
		cjne		@r1,#1,nosv1	;?????
...
yesbuf:		;записать   A3 в буфер
		call		resa_hl;dptr-
		call		loA3_X	;адрес след €чейки внешн озу храним
									;в €чейке saven_bl
		call		save_hl
		mov			r0,#diap
		mov			a,@r0
...
no0chkl:	jmp		kkout6
;?never here
		mov		P3IF,#00;		об€зательно сбросить оп€ть приходит на обработку прерывани€
		mov		P3,#0c0h
		reti
keyb1:	;IE6
;;;;;
...
;;;;;
		...
		push	dph
		 ...
		 call	z_01s;0,1s
				...
				;т к в соотв €ч буфера индикации пишетс€ 20h(пробел)
		mov		r0,#marker
		mov		a,#bufind
...
diinc:	call		clmassix
		...
		mov		r0,#rez_A0
		call		clear4
		clr		bitnul		;гасить светодиод Ќ”Ћ№	
		call		loadUS
		jmp		izmfun
...
knvich:	;12.02.07
		...
		mov		a,#4 
		subb		a,@r0
		jc		vit2_10;не нажимать кнопку ¬џ„ R T=10ms,2ms
		jb		biteizm,vit2_10;не нажимать кнопку ¬џ„ во врем€ теста
		jb		bitvi11,onvich
		clr		bitizm
		setb		bitvich
...
onvich:	clr		bitvich	;???
		...
		mov		@r0,#0
		jmp		izmpaT
; изм упр слова при нажатии »«ћ	d3(k5 p4),d3(u5 p5)
...
izman:	jb		biteizm,ontest	
		jb		knizm,knizmon
izman1:
		setb		knizm		;уст реж »«ћ ¬ Ћ
		;	d3(k5 p4),d3(u5 p5)заслать us
		call		louizm 
		jmp		izmfun


knizmon:	clr		knizm		;уст реж »«ћ ¬џ Ћ
...
;т е сохранить ее в €чейке дл€ мигани€ в противном случае
;при сдвиге курсора мигани€ вправо переносилась цифра с предыдущего знакоместа
		mov		r0,#marker
		mov		a,#bufind
		add		a,@r0			;a=bufind+marker
		mov		r1,a
...
;необходимо не портить цифру в следуюем справа знакоместе
;т е сохранить ее в €чейке дл€ мигани€ в противном случае
;при сдвиге курсора мигани€ вправо переносилась цифра с предыдущего 
;	знакоместа ->B=+00000
		mov		r0,#marker
		mov		a,#bufind
		add		a,@r0			;a=bufind+marker
...
chanZ:	mov		p3,#0feh
		mov		a,p3
		jb		acc.6,chanma
			;изм знак в старшем разр€де на противоположн
		mov		a,bufind+3
		cjne		a,#2bh,noplus
		mov		bufind+3,#2dh		;+->-	
		jmp		viout;outF
...
findmem:	jnb		bitmem,rnnmemb
		mov		r1,#level							;-> кн ѕјћя“№ (ќ„»—“ ј ->«јѕ»—№-> ѕ–ќ—ћќ“–)
		cjne		@r1,#11h,nomem11
...
labelL:	
		;;;;;
		mov			p3,#0dfh
		jnb			p3.6,moutF;в реж очистка полна€ не отрабатывать ¬џ„ R
		
		;ѕќЋЌјя->ЅЋќ  00(level 21->22)	???
		mov		r0,#level
		cjne		@r0,#21h,nobl21
		mov		@r0,#22h		;->ЅЋќ  
		mov		dptr,#teBL
...
no0chk:	mov		P3IF,#00;		об€зательно сбросить оп€ть приходит на обработку прерывани€
		mov		P3,#0c0h
		reti
...
;		mov	p_out,#0DAh
		...
		setb		bitmenu
		clr		bitoll
		mov		dptr,#teTESI 	;в буф2 записать “≈—“-»«ћ≈–
		call		lotext
		anl		bufind+11,#03h	;погасить зап€тые
		call		ind
		mov		r0,#vichR
		mov		@r0,#11h
		jmp		onmem11
...
hvi1113:	call		lotext	;в буфер 2 записать текст в соотв с состо€нием
		jmp		onmem11;goto7
...
nohnot2:	;в буфере 1 зажечь <=0,1s
		...
		call		ind
		jmp		loadB
nohnot1:	cjne	@r1,#4,nohnot2	;ввод кнопка “  
...
;кнопка ѕјћя“№			???
knMEM:	jb		bitklb,bout7	;в реж калибровки не нажимать пам€ть
		clr		bitizm		;off IZM
		mov		dptr,#teLIST	;CLE
		call		lotext		;в буф2 записать ќ„»—“ ј
		setb		bitmem		;нажата кнопка ѕјћя“№
		anl		bufind+11,#0efh	;убрать точку с 1го знакоместа
		orl		bufind+11,#01	;зажечь светодиод пам€ть
		call		ind
		mov		r0,#level
		mov		@r0,#13h;11h
...
labelK:
		jnb	bitmem,rnomemk		;кн пам€ть (ввод и <-)
			;;;;;;;
		mov	p3,#0dfh;(код кнопки меню)
		jnb	p3.7,offmenum
			;;;;;;;
		mov	p3,#0efh
		jnb	p3.7,outkmem		;отжатие кн пам€ть на любом уровне обработки пам€ти
		mov	r0,#level
...
chaibl:	mov		nblok,a
			   	 ;очистить блок пам€ти с набранным номером
...
goonoff:
		...
		mov		a,@r0
		jnb		acc.7,goonoff
		clr	bitmem	;сбр признак пам€ти 
		setb	bitizm
		;после очистки блока сохран€ем в saven_bl 
		;адрес второго с конца измерени€ поэтому добавл€ем 8
...
bl_li:	mov		@r0,#41h		; ЅЋќ  00(«јѕ»—№ » ѕ–ќ—ћќ“–)
		jmp		loblok
...
nom32:	cjne		@r0,#33h,nom33
		...
		;запомнить номер €чейки конца блока
		mov	@r0,#34h
INTn:		mov		r7,#10
		mov		r0,#adec
		call	clearN
...
nmem_menu:	jmp		out7			;нажата пам€ть в режиме меню
...
vvod43:	mov		@r0,#44h
		mov		a,nelem;вывод пор€дкового номера в блоке 
		inc		a
		mov		abin+3,a
...
noli_43:	cjne		@r0,#44h,vvosta
		...
		jmp			li42_43;вывод значени€ следующего в блоке
...
vv21_23:	mov		p3,#0f7h		;vvod
		jb		p3.7,onmem12
		cjne		@r0,#21h,test22
		;mov		dptr,#tabt11;заслать ус 11ј
tests:	call		lotpus
		setb		biteizm;запомнить режим “»
		setb		bitizm
		clr		bitmenu;обнулить т к не будет индикации измерени€	
		jmp		goto7;onmem11

test22:	cjne		@r0,#22h,test23
		;	mov			dptr,#tabt9;заслать ус 9ј
		jmp		tests	

test23:
		;	mov			dptr,#tabt7;заслать ус 7ј
		jmp		tests	
...
;и запись во внешнее озу(0000h)
		...
		mov		r3,#00
		call	copyPP
		;внести изменени€ во внешн озу
		;KD=00 01 86 A0	(KD=1)
		;коэфф *100000 и переведенный в hex формат
		;согласно адресов диапазонов и адресов €ч вн озу 
		mov		r1,#2ch
		mov		a,#0;57,6
		jnb		bitv19,spwri
		mov		a,#1;19.6
...
;стереть страницу пап прогр 
		...
		mov		r7,#0ffh
		call	cle256
;и запись в €чейки коэфф пам€ти прогр (3000h) 
		mov		r7,#0ffh
	...
	mov	RSTSRC,#10h	;??reset
		...
		clr	TR1	;stop Timer1
		jb		bitv19,outsko1	;загр “1 в соотв с установленной скоростью
		mov			TH1,#244		;57.6
		jmp			outsko
...
;	знакоместа
		...
		clr		bitmig
		mov		a,movleft
		cjne		@r0,#6,onmem13;;нажата кн <- при вводе цифр ѕј–ќЋ№ 000
		inc		@r0
		jmp		goto7
;;;;;;;

vvkali52:	;введено значение дл€ калибровки
...
poin3:	inc		@r0
;нажат ввод ј =000000 запомнить константу ј
nopoin7:	mov		p3,#0f7h
		mov		a,p3
		jb		acc.7,gogo7;out7
		mov		r0,#vichR
...
pozB:		;mov		r0,#konstB
		...
		mov		r1,#marker	 ;B=000000-> ¬џ„»—Ћ»“№
		mov		@r1,#0ffh
		mov		r0,#vichR
		mov		@r0,#11h
		jmp		lovich	   ;3.01.07¬џ„»—Ћ»“№
...
endkalib:;KD=Ak/A3
		jb		bifl_kt,yesvi_kr;выч коэфф  м
		mov		r0,#rez_A3
		call		resar2
		...
		call		ldc_ltemp			;r8..r11=8
		call		flmul
		call		ftol		;r2..r5->hex коэфф KD (–≈« »«ћ „»—Ћќ 6разр€дное)

		mov		r0,#koefKD
		call		saver2
		;запись в рпзу в соотв €чейку
...
;2.2.07
		mov		r1,#diap
		cjne		@r1,#5,fulkalib
		;jb		bifl_kt,yesvi_km;выч коэфф  м
		setb		bifl_kt
		;call		loadus
		mov		dptr,#tabt7;заслать ус 7ј	
		clr		a
...
out_kali:	call		z_1s
		call		z_1s
		jmp		goto7
yesvi_km:		;выч коэфф  м=(10**5/A2)*K7/K5
		mov		r2,#50h			;-11a
		mov		r3,#14h			;k7
		call		rebyte2				;r2..r5- hex
		call		altof			;r2..r5->float
		...
		call	ldc_ltemp			;r8..r11=8
		call	flmul				;r2..r5
		call	ftol			;r2..r5->hex коэфф Km(–≈« »«ћ „»—Ћќ 6разр€дное)
		mov			r0,#koefKD;2ое исп €чейки koefKD
		call		saver2
		;запись в рпзу в соотв €чейку
		mov		r1,#0			;счит рпзу во внешн озу
		mov		r2,#50h
...
vichmean:	
					;;;;;;;;;;
		call		adrblok;dptr-адр €чейки 1й соотв блока 
		mov			a,dpl
		mov			r7,a
		mov			a,dph
		mov			r6,a
...
oldmean:
		...
		mov			@r0,#0
meng:		call		adrblok;dptr-адр €чейки 1й соотв блока 
		mov			a,dpl
		mov			r7,a
		mov			a,dph
		mov			r6,a
		...
		;;;;;;;;;;;		
nextmea:	call		loahl_r2	;из €ч внешн озу ->r2..r5
		call		altof;->float
		call		move28			;r8..r11
		mov			r0,#work
		call		resar2
		...
		jmp			meng
outmea:		
		call		loahl_r2	;из €ч внешн озу ->r2..r5
		call		altof;->float
		call		move28			;r8..r11
		mov			r0,#work
		call		resar2
...
loahl_r2:	movx		a,@dptr		;из €ч внешн озу ->r2..r5
		mov			r2,a
		inc			dptr
		movx		a,@dptr
		mov			r3,a
...
;с конца блока адрес 2го измерение
		...
		mov		r0,#work	;блок анализируетс€ с конца 1€ измерение загружаетс€ в work
		...
		mov		r0,#chblok;volume
		mov		@r0,#0
ming:		call		adrblok;dptr-адр €чейки 1й соотв блока 
		mov		a,dpl
		mov		r7,a
		mov		a,dph
		mov		r6,a
...
;с конца блока адрес 2го измерение
		...
		mov		r0,#chblok;volume
		mov		@r0,#0
maxg:		call		adrblok;dptr-адр €чейки 1й соотв блока 
		mov		a,dpl
		mov		r7,a
		mov		a,dph
		mov		r6,a
