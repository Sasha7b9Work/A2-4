...
;;Добавлено К0,К1 в RS (откл/вкл. кор-ки 0) октябрь 2009
;;выдача в RS измерений в формате плав. точки (2C "," вместо 2E ".")
;;с 7F 0A в начале и 0A в кач-ве разделителя 25.11.09
;;выдача в RS cостояния прибора с 7F 0A в начале и 0A в конце 25.11.09
;;выдача в RS памяти с 7F 0A в начале 
;;и 0A в кач-ве разделителя 25.11.09
;;исправлены H0-2
;;переключение скорости RS	16.12.09
;;к-ды T0-5 (из T5 возвращается неправильно  исправлено 21.12.09
;;убрана выдача 200 байт в RS после команд T4,T5
;;сделана выдача в RS измерений на диапазонах 2 мс и 10мс как
;;и на остальных	21.12.09
;;исправлен T3 (без вкл. "измер." выдавал ERR 5 -- убран анализ
;; p1.1 на метке anlp1:) 24.12.09
;; сделана выдача 4-х 16-ных байт в RS на 2 mS
;; в виде порядок + 3байта из Rez_A3	26.01.10
;; перегрузка--1-й байт 41h (не работает на 2 и10 мс--
;; не включается bitoll) метка FF (переставить анализ
;; bitoll в ветку 2 мс)
;; Для игнорирования в режиме "ПАМЯТЬ--Статистика" кнопки "-->"
;; введено по метке labN_1: игнорирование по B0=61,62,63H
...
saven_bl3	data  0c2h	;копия адреса последней записанной ячейки блока3 
	   ;c4 1bait
;igor---
byte_kor	data	0c4h	;признак вкл/откл кор-ки
;----
...
extcle:	clr	a
		...
		mov		@r0,#0ffh
;--------------------------------------------
			mov		r0,#byte_kor	;igor
			mov		@r0,#0			;
;---------------------------------------------
after_2:	;?here 28.10.07
		mov		a,#0ffh
		mov		p5,a
		mov		r1,#parT
		mov		@r1,#2			;Tizm=1s
...
gopart:
		...
		mov		r1,#chmasN	;суммир-ние n значений =20
		cjne		@r0,#1,noparT1
	;РЕЗ+РЕЗ
		mov		r0,#reacp
...
moveR3_:
			clr	EA	;?
;			mov	A,#rez_R	;Ame,..; bufind - no use: for fastest isn't screen output
;			mov	p_out,A
;			mov	cnt_out,#4
...
;;;24/12/09		mov		nuerr,#35h
;;;24/12/09		call		error	;ошибка 5 перегрузка на диап 10-2,10-3	
...
md_3:
md_5:
;;;	jnb	is_rs,me_5		;out RS rez_A3
		jb	is_rs,md_51		;out RS rez_A3
		jmp		me_5
md_51:
;	jb	bitvi11,me_5	;rez_R is out
				mov	r1,#parT	;;;28.12.09
	jnb	bitoll,moveA3_	;бит  перегрузки
		;бит  перегрузки
	mov	r0,#bufind	;20.11.09#rez_A3#rez_A3
;;;---28.12.09--------
				cjne @r1,#6,moveR0_no2ms
				mov	r0,#rez_A3
moveR0_no2ms:
;;;---28.12.09--------
	mov	@R0,#41h
...
moveA3_:
;;;------ 28.12.09 --- вернуть на 2 мs выдачу в RS по старому ---
			cjne @r1,#6,moveA3_no2ms
		...
		inc	r0
			...
			mov	A,#bufind				;;#rez_A3	;Ame,..; bufind - no use: for fastest isn't screen output
			mov	p_out,A
			mov	cnt_out,#4
			mov	SBUF0,#0Ah	;#7Fh	;was cmd_rez
			jmp		me_5
moveA3_no2ms:
;;;------  28.12.09 -----------
;------------------ 23.11.09---------------------
...
;					inc	r1
		...
		mov	r0,#bufind
		mov	@r0,#2bh
		jnb	znmat,nozzmiRS;bitznak,nozzmi
		mov	@r0,#2dh
nozzmiRS:	mov	r1,#diap
		mov	r0,#bufind+9
...
;;;			mov	@r0,A
		...
		jb	biteizm,seegoonRS
		mov	@r0,#31h		 ;10-10
			inc	r0
;;		dec	r0
		mov	@r0,#30h
;;		inc	r0
	mov	cnt_out,#11	;20.11.09#4
		jmp	seegoonRS
nodi8RS:	cjne	a,#9,nodis9RS
		jb	biteizm,seegoonRS
		mov	@r0,#31h	   ;10-11
			inc	r0
;;		dec	r0
...
;------------------------------------------------
	...
	mov	A,#bufind	;20.11.09#rez_A3	;Ame,..; bufind - no use: for fastest isn't screen output
	mov	p_out,A
;	mov	cnt_out,#10	;20.11.09#4
	mov	SBUF0,#0Ah	;#7Fh	;was cmd_rez

;-----------28.12.09---------
		jnb	bitoll,me_5	;бит  перегрузки		mov	r0,#bufind	;20.11.09#rez_A3#rez_A3
		mov	r0,#bufind
		mov	@R0,#41h
;----------28.12.09
me_5:			;??????????????????
		...
		call	loA3_X;адрес след ячейки внешн озу храним в ячейке saven_bl
		call		save_hl
		mov			r0,#diap
		mov			a,@r0
		mov			r1,#nblok
...
;-----------------------------------------
			...
			cjne		@r1,#41h,labLov			;Блок при ПРОСМОТР
			jmp		out6

labLov:
			cjne		@r1,#31h,labLLL			;Блок при ЗАПИСЬ
			jmp		out6
...
labN_1:	
;--------------------------------------------
;В режиме статистика min,max,sred игнорировать -->
;7.09.10
			cjne		@r0,#61h,labN_1_1
			jmp		out6
labN_1_1:
			cjne		@r0,#62h,labN_1_2
			jmp		out6
labN_1_2:
			cjne		@r0,#63h,labN_1_3
			jmp		out6
labN_1_3:
;--------------------------------------------
		mov		r0,#marker
...
hnot1:	mov		p3,#0f7h
		...
		cjne	@r1,#5,nohnot1	;r1=load	load=5(10s)?
...
knMEM:	
		...
		jb		bitklb,bout7	;в реж калибровки не нажимать память
		clr		bitizm		;off IZM
		mov		dptr,#teLIST	;CLE
...
;;;;;;;;;;;;;;;;
		...
		jmp		goto7						 
left21:	
;;;;					mov		a,movleft;уст в левую позицию
;;;;					mov		@r0,a	
		jmp		goto7
...
;		jnc		loadB2rs
;		mov		@r1,#1;N=1	   ;T>=2
;bout7rs:	jmp		tr9rs		;goto7;out7
;
;loadB2rs:	cjne	@r1,#2,bout7rs
;		mov		@r1,#2;N=2
;;		jmp		goto7;out7
...
mtk:		cjne	@R0,#'K',mth
		inc	r0
		cjne	@R0,#0,mtk1
			mov	r1,#byte_kor		;откл. корр.0
			mov		A,#1
			mov	@R1,A						
		jmp	tr9
mtk1:		
			mov	r1,#byte_kor		;вкл.корр.0
			clr	A
			mov	@R1,A						
		mov		r0,#chinkor
		mov		@r0,#0;1;
