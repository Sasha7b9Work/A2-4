...
//	char data lated[]_at_ 0x7f;
//	char idata firsti[]_at_ 0x80;
// 	char idata latei[]_at_ 0xff;
	...
	char data PP;
	char data	address;
	char data	count_mas;//счетчик заполнения массива

	bit  znak;	//бит знака	
	bit  over_mas;//бит заполнения массива из 10 значений	

	char data  bufind [32]_at_ 0x2e;  //2e..4e
...
//	char data late[]_at_ 0x7f;

 char buffer [10];
  int n, a=0xfe, b=3;
  //unsigned long  k;
//XRAM внешнее озу
	 float   xdata SUMM;//_at_ 0x0000;	
	 
	 char floatbuf [10];
	 float   xdata MASS[10];//[2]//_at_ 0x0003;
//	 float  xdata MAS1[3]_at_ 0x0009;
//	 float   xdata MAS2[4]_at_ 0x000f;
////////////////////////////
 // data int begin _at_ 0x00;
 // char data dass _at_ 0x4b;

 //char idata begi _at_ 0x80;
 //char idata iasse _at_ 0xfe;

//char pdata pass _at_ 0x80;
//char pdata ppass _at_ 0xff;
//char pdata pass1 _at_ 0x00;

 //unsigned char xdata rr[11]_at_ 0x80;
 //unsigned short xdata meagement[10]_at_ 0x8000;
...
///////////////////////////////////
	...
	if (dj!=0x00){
		...
		znak=0;//bufind[6]=0x2b;//bufind[5]=0x2b;//bufind[4]=0x2b;	
	}
	else
		{
		...
		znak=1;//bufind[6]=0x2d;//bufind[5]=0x2d;//bufind[4]=0x2d;
	}
///////////////////////////////////
//преобразование и выдача на индикатор
	...
	//rez=sprintf(buffer,"%lf",k);//
	rez=atof(buffer);//плавающй результат
	//if (znak==1)
	//	rez=-rez;
//	rez=rez/3;
//	rez=rez*3;
...
// скользящий массив из 10х значений
	MASS[9]=MASS[8];  //MASS[8]->MASS[9]
	MASS[8]=MASS[7];  //MASS[7]->MASS[8]
	MASS[7]=MASS[6];  //MASS[6]->MASS[7]
	MASS[6]=MASS[5];  //MASS[5]->MASS[6]
	MASS[5]=MASS[4];  //MASS[4]->MASS[5]
	MASS[4]=MASS[3];  //MASS[3]->MASS[4]
	MASS[3]=MASS[2];  //MASS[2]->MASS[3]
	MASS[2]=MASS[1];  //MASS[1]->MASS[2]
	MASS[1]=MASS[0];  //MASS[0]->MASS[1]

	MASS[0]=rez;	  //рез -> MAS0[0]
	SUMM=0;
	SUMM=MASS[0]+ MASS[1]+MASS[2]+MASS[3]+MASS[4]+MASS[5];
	SUMM=SUMM+MASS[6]+ MASS[7]+MASS[8]+MASS[9] ;

	if (over_mas!=1){ 
		count_mas++;
		SUMM=SUMM/count_mas;
		if (count_mas==10)
			over_mas=1;	 //массив заполнился
	}
	 else
	 	SUMM=SUMM/10.0;
...
void d_second (void)
{
	...
	for (count = 0xff; count != 0; count--) 
		for (hcount=0xff;hcount!=0;hcount--)
			;
}
