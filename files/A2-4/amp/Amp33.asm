...
goonoff:
			...
			;адрес второго с конца измерения поэтому
			;добавляем 8;;
...
gooff1:
			...
			mov		a,@r0
			jnb		acc.7,gooff1
			mov			dptr,#158h;начало 1 го блока
			;после очистки блока сохраняем в saven_bl 
			;адрес второго с конца измерения поэтому
			;добавляем 8;;
			call		save_hl;подготовка записи в начало блока
...
gooff2:
			...
			mov		a,@r0
			jnb		acc.7,gooff2
			mov			dptr,#478h;начало 2 го блока
			call		save_hl;подготовка записи в начало блока
;3й блок
			mov			dptr,#790h
				mov			r1,#work		;обьем блока
...
gooff3:
			...
			mov		a,@r0
			jnb		acc.7,gooff3
			mov			dptr,#798h;начало 3 го блока
				;после очистки блока сохраняем в saven_bl 
			;адрес второго с конца измерения поэтому
			;добавляем 8;;
			call		save_hl;подготовка записи в начало блока
...
gooff4:
			...
			mov		a,@r0
			jnb		acc.7,gooff4
			mov			dptr,#0ab8h;начало 4 го блока
			;после очистки блока сохраняем в saven_bl 
			;адрес второго с конца измерения поэтому
			;добавляем 8;;
			call		save_hl;подготовка записи в начало блока
